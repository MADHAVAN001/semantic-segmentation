...
epochs, nfilters, dropout, kernel_size, batch_norm, start_epoch_prune = dataloader.coco.get_model_hyperparams(cfg)
...
begin_step_prune = np.int(steps_per_epoch_train * start_epoch_prune)
end_step_prune = np.int(epochs * steps_per_epoch_train)

unet_inst = models.unet.uNetModel(img_width, img_height, n_chan, num_classes, nfilters, kernel_size, dropout, batch_norm)
tensorflow.keras.models.save_model(unet_inst.model,"./curr.mod")
print(type(unet_inst.model))
#pruning_params = { 'pruning_schedule' : sparsity.PolynomialDecay(initial_sparsity=0.5, 
#                    final_sparsity=0.9, begin_step= begin_step_prune,end_step=end_step_prune,frequency=40)}
pruning_schedule = sparsity.PolynomialDecay(initial_sparsity=0.5, final_sparsity=0.9, begin_step= begin_step_prune,end_step=end_step_prune,frequency=40)
pruned_model = tensorflow.keras.Sequential()

for x in unet_inst.model.layers[1:]:
    print(x.name)
    print(type(x))
    if("conv2d" in x.name):
        pruned_model.add(sparsity.prune_low_magnitude(x, pruning_schedule))
    else:
        pruned_model.add(x)
pruned_model.summary()

new_model = sparsity.prune_low_magnitude(unet_inst.model, **pruning_params)
new_model.summary()
...
if train:
    callbacks = [
        ...
        pruning_callbacks.PruningSummaries(log_dir="./workspace/prune.log")
    ]
